...
// import { Search, ShoppingCart, Star, Heart, ChevronLeft, ChevronRight } from 'lucide-react';
// import { useCart } from '../../context/CartContext';
// import ReactPaginate from 'react-paginate';
// import { supabase } from '../../api'; // Đảm bảo đường dẫn import đúng
// import { Link } from 'react-router-dom';

// // --- MOCK DATA Categories (Vẫn giữ lại để lọc) ---
// const categories = ["All", "Burger", "Pizza", "Pasta", "Chicken", "Fries", "Drinks"];

// // Component Hiển thị Item (Giữ nguyên)
// const ProductItems = ({ currentItems, addToCart }) => {
//     return (
//         <>
//             {currentItems && currentItems.map((item) => (
//                 <motion.div
//                     // ... các props của motion.div giữ nguyên
...
//                         <button className="absolute top-3 right-3 w-8 h-8 bg-white rounded-full flex items-center justify-center text-gray-300 hover:text-red-500 shadow-sm transition-colors z-10">
//                             <Heart size={16} fill="currentColor" />
//                         </button>

//                         {/* 2. Bọc ảnh bằng Link để click vào được */}
...
//                         <button
//                             onClick={(e) => {
//                                 e.stopPropagation(); // Ngăn click lan ra ngoài (nếu có)
//                                 addToCart(item);
//                             }}
//                             className="bg-black text-white w-10 h-10 rounded-full flex items-center justify-center shadow-lg hover:bg-[#9e1c20] transition-colors active:scale-90"
//                         >
...
//     const [products, setProducts] = useState([]); // State lưu trữ dữ liệu từ Supabase
...
//                     .from('Products') // Tên bảng trong Supabase
//                     .select('*');
...
//     // --- LOGIC PHÂN TRANG & FILTER ---
//     const [itemOffset, setItemOffset] = useState(0);
//     const itemsPerPage = 8;

//     // 1. Lọc sản phẩm (Dùng state products thay vì allProducts mock)
...
//     // 3. Reset về trang 1 khi thay đổi bộ lọc
//     useEffect(() => {
//         setItemOffset(0);
//     }, [activeCategory, searchTerm]);

//     // 4. Xử lý khi bấm chuyển trang
...
//             {/* ... (Phần Header Banner và Filter giữ nguyên) ... */}
...
//                         <input type="text" placeholder="Search food..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full bg-[#f4f1ea] py-3 pl-12 pr-4 rounded-full focus:outline-none focus:ring-2 focus:ring-[#9e1c20]/50 transition-all" />
...
//                                 <ProductItems currentItems={currentItems} addToCart={addToCart} />
//                             ) : (
...
//                             breakLabel="..."
//                             nextLabel={
//                                 <span className="flex items-center justify-center w-10 h-10 rounded-full bg-white shadow-md hover:bg-[#9e1c20] hover:text-white transition-colors">
                                ...
                                currentItems.map((item) => (
                                    ...
                                    >
                                        ...
                                        <div className="relative bg-[#fff8f0] rounded-2xl p-4 mb-4 flex items-center justify-center h-48 group-hover:bg-[#fff5eb] transition-colors">
                                            ...
                                            {/* Link bao quanh ảnh */}
                                            <Link to={`/product/${item.id}`} className="block w-full h-full flex items-center justify-center">
                                                <img 
                                                    src={item.image} 
                                                    alt={item.name} 
                                                    className="w-36 h-36 object-contain group-hover:scale-110 transition-transform duration-500 drop-shadow-md cursor-pointer" 
