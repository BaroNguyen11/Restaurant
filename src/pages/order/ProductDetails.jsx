...
const ProductDetails = () => {
        ...
        const fetchProductDetails = async () => {
            ...
            try {
                ...
                if (reviewsData && reviewsData.length > 0) {
                    const totalScore = reviewsData.reduce((sum, item) => sum + item.rating, 0);
                    const avg = totalScore / reviewsData.length;

                    // 👇 QUAN TRỌNG: Cập nhật state rating bằng số tính được
                    setRating(avg);
                } else {
                    // Nếu chưa có review, mặc định là 5 hoặc 0 tùy bạn
                    setRating(0);
                }
    ...
    if (!product) return (
        <div className="h-screen flex flex-col items-center justify-center text-center">
            ...
            <Link to="/order/order_food" className="text-[#9e1c20] hover:underline">Back to Menu</Link>
        </div>
    );
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 mb-24">
                    ...
                    >
                        ...
                        <div className="flex items-center gap-4 mb-6">
                            ...
                            <span className="text-sm text-gray-500 font-medium">({reviewCount || 10} Customer Reviews)</span>
                        </div>
                    ...
                    <div className="flex gap-8 border-b-2 border-gray-100 mb-8">
                        {['Description', 'Reviews'].map((tab) => {
                            ...
                            return (
                                ...
                                >
                                    {tab} {key === 'revi' && <span className="text-sm ml-1">({reviewCount})</span>}
                                    {isActive && (
